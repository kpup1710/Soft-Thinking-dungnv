...
class LlamaModel(nn.Module):
    ...
    def forward(
        ...
        # begin of soft thinking
        # ==========
        if forward_batch.concept_embeddings is not None:
            # Projection concept token: pre-computed [B, D] embedding
            hidden_states = forward_batch.concept_embeddings
        elif forward_batch.topk_probs is not None and forward_batch.topk_indices is not None:
            if self.tp_size > 1:
